...
class MDTraj:
    ...
    def calculate_rmsd(self, rmsd_select):

        selected_atoms = self.u.select_atoms(rmsd_select)
        ref_positions = selected_atoms.positions

        tasks = [(frame_index, ref_positions, selected_atoms) for frame_index in range(len(self.u.trajectory))]

        with Parallel(n_jobs=-1) as parallel:
            results = parallel(delayed(self.calculate_rmsd_frame)(frame_index, ref_positions, selected_atoms) for frame_index, ref_positions, selected_atoms in tqdm(tasks, desc="Calculating RMSD"))
    ...
    def plot_rmsd(self, backbone=None, color=None):
        ...
        #plt.xlim(x_limits)
        plt.ylim(top=4) # backbone
        #plt.ylim(top=10) # ligante
        plt.tight_layout()
    ...
    def play(self):
        ...
        """customizar parametros de plot e output dos metodos calculate_rmsd() e plot_rmsd()"""
